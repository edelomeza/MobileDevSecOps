...
@Composable
fun EmpleadoCrearScreen(
    onNavigateBack: () -> Unit,
    onEmpleadoCreado: () -> Unit,
    onError: () -> Unit,
    onSessionExpired: () -> Unit,
    viewModel: EmpleadoCrearViewModel = koinViewModel()
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.events.collect { event ->
            when (event) {
                is EmpleadoCrearEvent.NavigateBack -> onNavigateBack()
                is EmpleadoCrearEvent.EmpleadoCreado -> onEmpleadoCreado()
                is EmpleadoCrearEvent.Error -> onError()
                is EmpleadoCrearEvent.SessionExpired -> onSessionExpired()
            }
        }
    }
