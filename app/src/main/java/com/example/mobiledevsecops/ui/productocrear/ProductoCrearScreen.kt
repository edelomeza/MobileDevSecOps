...
@Composable
fun ProductoCrearScreen(
    onNavigateBack: () -> Unit,
    onProductoCreado: () -> Unit,
    onError: () -> Unit,
    onSessionExpired: () -> Unit,
    viewModel: ProductoCrearViewModel = koinViewModel()
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.events.collect { event ->
            when (event) {
                is ProductoCrearEvent.NavigateBack -> onNavigateBack()
                is ProductoCrearEvent.ProductoCreado -> onProductoCreado()
                is ProductoCrearEvent.Error -> onError()
                is ProductoCrearEvent.SessionExpired -> onSessionExpired()
            }
        }
    }
