...
fun UsuarioActualizarScreen(
    ...
    viewModel: UsuarioActualizarViewModel = koinViewModel(parameters = { parametersOf(id, nombre, correo, rowVersion) })
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.events.collect { event ->
            when (event) {
                is UsuarioActualizarEvent.NavigateBack -> onNavigateBack()
                is UsuarioActualizarEvent.UsuarioActualizado -> onUsuarioActualizado()
                is UsuarioActualizarEvent.Error -> onError()
                is UsuarioActualizarEvent.SessionExpired -> onSessionExpired()
            }
        }
    }
