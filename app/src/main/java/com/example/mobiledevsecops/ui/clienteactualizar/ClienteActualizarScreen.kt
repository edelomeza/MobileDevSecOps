...
@Composable
fun ClienteActualizarScreen(
    params: ClienteActualizarParams,
    onNavigateBack: () -> Unit,
    onClienteActualizado: () -> Unit,
    onError: () -> Unit,
    onSessionExpired: () -> Unit,
    viewModel: ClienteActualizarViewModel = koinViewModel(parameters = { parametersOf(params) })
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.events.collect { event ->
            when (event) {
                is ClienteActualizarEvent.NavigateBack -> onNavigateBack()
                is ClienteActualizarEvent.ClienteActualizado -> onClienteActualizado()
                is ClienteActualizarEvent.Error -> onError()
                is ClienteActualizarEvent.SessionExpired -> onSessionExpired()
            }
        }
    }
